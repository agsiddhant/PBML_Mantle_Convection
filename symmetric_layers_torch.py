...
class SymmetricConv2d(nn.Conv2d):
    def __init__(
        ...
        if symmetry is None:
            ...
            assert (
                sum(list(symmetry.values())) <= self.out_channels
            ), "Number of symmetric channels exceeds number of out channels"
            self.unique_out_channels = (
                self.out_channels
                - symmetry["h"] // 2
                - symmetry["v"] // 2
                - 3 * symmetry["hv"] // 4
        ...
        if symmetry is None:
            ...
            for key, val in symmetry.items():
                if key not in ("h", "v", "z", "hv", "hz", "vz", "hvz"):
                    raise ValueError(
                        "Unknown key, use only 'h','v','z','hv','hz','vz','hvz'"
                    )
                    ...
                    raise ValueError(
                        "Number of symmetric hv filters must be divisible by 8"
                    )
            # sanity check: number of symmetric filters must be <= number of filters
            assert (
                sum(list(symmetry.values())) <= self.out_channels
