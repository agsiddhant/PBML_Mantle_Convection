...
def get_indices_time(data_dir, an, is_init=False, debug=True, roll_forward=1):
    ...
    for si, sim in enumerate(sims):
        ...
        if check and si not in ignore_inds:# and (si==0 or si == 110):
            take_every = 1
            if debug:
                u      = torch.load(py_dir + "/e" + str(take_every) + "_uprev_data_select_init.pt").repeat(roll_forward*2,1,1,1)
...
class ADTimeDataset(Dataset):
    ...
    def __init__(self, data_dir, an, scale=True, load=False, is_init=False, p_pred=True, 
                 noise=0.0, debug=True, sims_vec=[], times_vec=[],roll_forward=1):
        ...
        for si, sim in enumerate(sims):
            ...
            if check and si not in ignore_inds and len(times)>1:# and (si==0 or si == 110):
                ...
                self.yc    = torch.load(py_dir + "/yc.pt") #.reshape(xc.shape[0],xc.shape[1])
                ...
                paras_nd = torch.tensor([raq_nd,fkt_nd,fkp_nd], dtype=torch.float64).view(3,1,1)

                if debug:
                    reps = max(1, int(roll_forward/2*2))
                    u      = torch.load(py_dir + "/e" + str(take_every) + "_uprev_data_select_init.pt").repeat(reps,1,1,1)
                    v      = torch.load(py_dir + "/e" + str(take_every) + "_vprev_data_select_init.pt").repeat(reps,1,1,1)
                    Tprev  = torch.load(py_dir + "/e" + str(take_every) + "_Tprev_data_select_init.pt").repeat(reps,1,1,1)
                    times  = torch.load(py_dir + "/times.pt")[:u.shape[0]]
                    if p_pred:
                        raise ValueError('p_pred is not implemented in debug mode')
                else:
                    u      = torch.load(py_dir + "/e" + str(take_every) + "_uprev_data.pt")[:760,...]
                    v      = torch.load(py_dir + "/e" + str(take_every) + "_vprev_data.pt")[:760,...]
                    if p_pred:
                        p  = torch.load(py_dir + "/e" + str(take_every) + "_pprev_data.pt")[:760,...]
                    Tprev  = torch.load(py_dir + "/e" + str(take_every) + "_Tprev_data.pt")[:760,...]
                    times = torch.load(py_dir + "/times.pt")[:u.shape[0]]
                
                for i, t in enumerate(times):
                    if len(sims_vec)>0:
                        check =  t in np.asarray(times_vec)[np.asarray(sims_vec)==sim[0]]
                    else:
                        check = True
    ...
    def __getitem__(self, idx):
        ...
        scaler = np.exp((self.paras[idx0][0:1,...]/10)*1.80167667 
                + np.log(self.paras[idx0][1:2,...])*0.4330392 
                + np.log(self.paras[idx0][2:3,...])*-0.46052953 )*5  
        
        t_weight = self.t_data[idx0].double()
        Tp       = self.x_data[idx0].double()
        y        = self.y_data[idx0].double()
        V        = eta_torch(self.paras[idx0][1:2,...], self.paras[idx0][2:3,...], 1.-self.yc, Tp)
        ...
        x = torch.cat((
                       self.xc,
                       self.yc,
                       torch.tensor((self.t[idx1]-self.t[idx0]), dtype=torch.float64).expand(1,Tp.shape[-2],Tp.shape[-1]),
...
def get_indices(data_dir, an, is_init=False, debug=True ,roll_forward=1):
    ...
    for si, sim in enumerate(sims):
        ...
        if check and si not in ignore_inds:# and (si==0 or si == 110):
            take_every = 1
            if is_init:
                i_vec  = torch.load(py_dir + "/e" + str(take_every) + "_i_vec_select_init.pt")
            else:
                if debug:
                    u      = torch.load(py_dir + "/e" + str(take_every) + "_uprev_data_select_snaps.pt")
                    i_vec = np.arange(u.shape[0])
                else:
                    i_vec  = torch.load(py_dir + "/e" + str(take_every) + "_i_vec_select.pt")
            for i_prev in i_vec:
                sims_vec.append(sim[0])
...
class NewADDataset(Dataset):
    ...
    def __init__(self, data_dir, an, scale=True, load=False, is_init=False, p_pred=True, 
                 noise=0.0, debug=True, sims_vec=[], times_vec=[], max_examples_percent_per_epoch=100):
        ...
        for si, sim in enumerate(sims):
            ...
            if check and si not in ignore_inds and len(times)>1:# and (si==0 or si == 110):
                ...
                if load:
                    ...
                    if len(times)> 700:
                        rest_steps = random.choices(rest_steps, k=min(500,rest_steps[-1]-200))
                    ...
                    for i in i_vec:
                        ...
                        self.y_data.append(y)
                        self.t_data.append(torch.tensor(6/(i+1)**0.25, dtype=torch.float64))
                        
                else:
                    if is_init:
                        u      = torch.load(py_dir + "/e" + str(take_every) + "_uprev_data_select_init.pt")
                        v      = torch.load(py_dir + "/e" + str(take_every) + "_vprev_data_select_init.pt")
                        if p_pred:
                            p      = torch.load(py_dir + "/e" + str(take_every) + "_pprev_data_select_init.pt")
                        Tprev  = torch.load(py_dir + "/e" + str(take_every) + "_Tprev_data_select_init.pt")
                        i_vec  = torch.load(py_dir + "/e" + str(take_every) + "_i_vec_select_init.pt")
                    else:
                        if debug:
                            u      = torch.load(py_dir + "/e" + str(take_every) + "_uprev_data_select_snaps.pt")
                            v      = torch.load(py_dir + "/e" + str(take_every) + "_vprev_data_select_snaps.pt")
                            Tprev  = torch.load(py_dir + "/e" + str(take_every) + "_Tprev_data_select_snaps.pt")
                            i_vec = np.arange(u.shape[0])
                            if p_pred:
                                raise ValueError('p_pred is not implemented in debug mode')
                        else:
                            u      = torch.load(py_dir + "/e" + str(take_every) + "_uprev_data_select.pt")
                            v      = torch.load(py_dir + "/e" + str(take_every) + "_vprev_data_select.pt")
                            if p_pred:
                                p      = torch.load(py_dir + "/e" + str(take_every) + "_pprev_data_select.pt")
                            Tprev  = torch.load(py_dir + "/e" + str(take_every) + "_Tprev_data_select.pt")
                            i_vec  = torch.load(py_dir + "/e" + str(take_every) + "_i_vec_select.pt")
                    ...
                    for i, i_prev in enumerate(i_vec):
                        if len(sims_vec)>0:
                            ...
                            check =  i_prev in np.asarray(times_vec)[np.asarray(sims_vec)==sim[0]]
                        else:
                            check = True
                        if check:
                            self.paras.append(paras)
                            ...
                            self.x_data.append(Tprev[i,...])
                            self.y_data.append(y)
                            self.t_data.append(torch.tensor(6/(i_prev+1)**0.25, dtype=torch.float64))
                    del u,v,Tprev

        self.num_examples = min(int(len(self.y_data)*max_examples_percent_per_epoch/100), len(self.y_data))
    ...
    def __getitem__(self, idx):
        ...
        if self.noise>0:
            n = torch.tensor(np.random.uniform(low=-1e-5, high=1e-5, #self.noise, high=self.noise, 
                                               size=(1,Tp.shape[-2]-4,Tp.shape[-1]-4)), dtype=torch.float64)
            Tp[:,2:-2,2:-2] = torch.clip(Tp[:,2:-2,2:-2] + n, 0.0, 1.35)
            
        y        = self.y_data[idx].double()
        V        = eta_torch(self.paras[idx][1:2,...], self.paras[idx][2:3,...], 1.-self.yc, Tp)
        V        = torch.clip(V,1e-08,1)

        if self.scale:
            scaler = np.exp((self.paras[idx][0:1,...]/10)*1.80167667 
                    + np.log(self.paras[idx][1:2,...])*0.4330392 
                    + np.log(self.paras[idx][2:3,...])*-0.46052953 )*5  
            x = torch.cat((
                           self.xc/4,
                           self.yc/4,
                           torch.log10(V)/8,
