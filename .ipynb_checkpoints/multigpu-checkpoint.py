...
def ddp_setup(rank, world_size, master_port):
    ...
    init_process_group(backend="nccl", rank=rank, world_size=world_size, timeout=datetime.timedelta(seconds=36000))
class Trainer:
    def __init__(
        self,
        ...
        self.dx_left_kernel   = torch.Tensor([-1,1,0]).double().unsqueeze(0).unsqueeze(1).unsqueeze(2).to(gpu_id)
        self.dy_top_kernel    = torch.Tensor([-1,1,0]).double().unsqueeze(0).unsqueeze(1).unsqueeze(3).to(gpu_id)
    ...
    def loss_fn(self, x_true, x_pred):
        ...
        if self.loss_scale:
            ...
            scaler = torch.clip(1./(maxs-mins), 1., 10.)
            bc_scaler = torch.ones_like(x_true) + 10.
            bc_scaler[:,2:-2,2:-2] = 1.
    ...
    def get_loss(self, gVTp, uvp, scaler, paras=None, yc=None):
        ...
        if  "fluidnet" in self.net:
            ...
            if self.model_AD is None:
                ...
                if self.p_pred:
                    loss   = (loss_u + loss_v + loss_p + loss_T)/4.
                else:
                    loss   = (loss_u + loss_v + loss_T)/3.
            
            if self.loss_type=="mass":
                loss += torch.mean(mass_consv)
            elif self.loss_type=="curl":
                loss += torch.mean(mass_consv[:,:,:,0]) \
            ...
            if self.p_pred:
                xc,yc,dt,raq_nd,fkt_nd,fkp_nd,V,T,u,v,p = torch.split(gVTp,1,dim=1)
            else:
                xc,yc,dt,raq_nd,fkt_nd,fkp_nd,V,T,u,v = torch.split(gVTp,1,dim=1)
    ...
    def _run_batch(self, gVTp, uvp, scaler, is_train, paras=None, yc=None):
        ...
        if is_train:
            ...
            if self.net == "convae":
                loss, loss_u, loss_v, loss_p, loss_T, mass_consv = self.get_loss_convae(gVTp, scaler, paras, yc)
            else:
                loss, loss_u, loss_v, loss_p, loss_T, mass_consv = self.get_loss(gVTp, uvp, scaler, paras, yc)
            loss.backward()
            self.optimizer.step()
        else:
            if self.net == "convae":
    ...
    def _run_epoch(self, epoch):
        ...
        for i, data in enumerate(self.train_data):
            ...
            losses = self._run_batch(gVTp, uvp, scaler, is_train=True, paras=paras, yc=yc)
            self.losses = [losses[j] + self.losses[j] for j in range(6)]
            t1 = time.time()
            if i%100==0:
                print(epoch, [float(self.losses[j]/(i+1)) for j in range(6)], t1-t0)
        ...
        with torch.no_grad():
            ...
            for i_cv, data in enumerate(self.cv_data):
                ...
                gVTp   = gVTp.to(self.gpu_id)
                uvp    = uvp.to(self.gpu_id)
                losses_cv = self._run_batch(gVTp, uvp, scaler, is_train=False, paras=paras, yc=yc)
...
def load_train_objs(rank, world_size, nn_dir, data_dir, levels, c_i, c_h, c_o, act_fn, r_p, loss_type,
                    use_symm, repeats, kernel, milestones, sims_vec, times_vec,
                    sims_vec_init, times_vec_init, use_skip=False, p_pred=False,
                    spectral_conv=False, dilation=1, a_bound=10, restart=False, 
                    advect=False, network="fluidnet", scale=True, noise=0.0, debug=False, blurr=False,
                    l2_reg=0.,dropout=0.,roll_forward=1,factor=2,multi_scales=[]):
    ...
    if network == "fluidnet" or network == "ifluidnet" :
        model_uvp = FluidNet(levels, c_i, c_h, c_o, rank, act_fn, r_p, loss_type, 
                             ...
                             spectral_conv=spectral_conv, blurr=blurr,drop_rate=dropout).double()
    elif network == "newfluidnet" :
        model_uvp = NewFluidNet(levels, c_i, c_h, c_o, rank, act_fn, r_p, loss_type, 
                             use_symm=use_symm, dilation=dilation, a_bound=a_bound,
                             repeats=repeats, use_skip=use_skip, f=kernel, p_pred=p_pred,
                             spectral_conv=spectral_conv, blurr=blurr,drop_rate=dropout,factor=factor).double()
    elif network == "multiscalenewfluidnet" :
        models = torch.nn.ModuleList()
        for _ in multi_scales:
            models.append(HalfNewFluidNet(levels, c_i, c_h, c_o, rank, act_fn, r_p, loss_type, 
                             use_symm=use_symm, dilation=dilation, a_bound=a_bound,
                             repeats=repeats, use_skip=use_skip, f=kernel, p_pred=p_pred,
                             spectral_conv=spectral_conv, blurr=blurr,drop_rate=dropout,factor=factor).double()
                         )
        model_uvp = MultiScaleNewFluidNet(nets=models, loss_type=loss_type, 
                                          device=rank, scales=multi_scales,
                                          p_pred=p_pred).double()
    elif network == "convae":
        model_uvp = ConvAE(levels, c_i, c_h, c_o, rank, act_fn, r_p, loss_type, 
                             use_symm=use_symm, dilation=dilation, a_bound=a_bound,
                             repeats=repeats, use_skip=use_skip, f=kernel, p_pred=p_pred,
                             spectral_conv=spectral_conv, blurr=blurr).double()
    elif network == "unet" or network == "iunet":
        model_uvp = Unet(levels, c_i, c_h, c_o, rank, act_fn, r_p, loss_type, 
                             use_symm=use_symm, dilation=dilation, a_bound=a_bound,
                             repeats=repeats, use_skip=use_skip, f=kernel, p_pred=p_pred,
                             spectral_conv=spectral_conv, blurr=blurr,drop_rate=dropout).double()
    ...
    if advect:
        model_AD = ADNet(device=rank, CN_max=2e+4).double()
        model_AD.eval()
    else:
        model_AD = None
    ...
    if restart:                
        ...
        model_uvp.load_state_dict(torch.load(nn_dir + str(epoch) + "_fluidnet_uvp.pt", map_location=torch.device('cuda', device)))
        epoch += 1
    ...
    for an in ["train", "cv"]:
        ...
        dataset_len = int((len(sims_vec[an])-len(sims_vec[an])%world_size)/world_size)
        start_ind = int(dataset_len*rank)
        end_ind   = int(dataset_len*(rank+1))
        print(rank, "splitting ", len(sims_vec[an]), " samples into ", world_size, " chunks of size ", end_ind-start_ind)
        ...
        if network == "unet" or network=="iunet":
            ...
            if debug:
                ...
                dataset_len = int((len(sims_vec_init[an])-len(sims_vec_init[an])%world_size)/world_size)
                start_ind = int(dataset_len*rank)
                end_ind   = int(dataset_len*(rank+1))
...
def main(rank: int, world_size: int, 
    ...
    dataset, dataset_init, model_uvp, model_AD, optimizer, scheduler, epoch = load_train_objs(rank, world_size, nn_dir, data_dir, levels, c_i, 
                                                                                        c_h, c_o, act_fn, 
                                                                                        r_p, loss_type, 
                                                                                        use_symm, repeats, kernel, milestones, sims_vec,
                                                                                        ...
                                                                                        scale=scale, debug=debug, blurr=blurr,
                                                                                        l2_reg=l2_reg,dropout=dropout,roll_forward=roll_forward,
                                                                                        factor=factor, multi_scales=multi_scales)
    small_batch = 2
    if world_size > 1:
        small_batch = 1
    train_data = prepare_dataloader(dataset["train"], batch_size-small_batch, world_size, rank)
    cv_data    = prepare_dataloader(dataset["cv"], batch_size-small_batch, world_size, rank)
    if dataset_init["train"] is not None:
        train_data_init = prepare_dataloader(dataset_init["train"], small_batch, world_size, rank)
        cv_data_init    = prepare_dataloader(dataset_init["cv"], small_batch, world_size, rank)
    else:
    ...
    trainer    = Trainer(model_uvp, model_AD, train_data, cv_data, 
                         ...
                         nn_dir, p_pred, debug, network, loss_scale, loss_derivative, 
                         roll_forward, epoch=epoch, loss_type=loss_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-lt", "--loss_type", type=str, help="loss type", default="curl")
    ...
    parser.add_argument("-l_de", "--loss_derivative", type=int, help="scale loss", default=0)
    ...
    parser.add_argument("-ad", "--advect", type=int, help="add advection to loss", default=0)
    ...
    parser.add_argument("-spectral", "--spectral_conv", type=int, help="use spectral conv", default=0)
    parser.add_argument("-mp", "--master_port", type=int, help="master_port", default=366)
    parser.add_argument("-l2", "--l2_reg", type=float, help="weight decay", default=0.)
    parser.add_argument("-d_r", "--drop_rate", type=float, help="dropout rate", default=0.)
    parser.add_argument("-roll", "--roll_forward", type=int, help="roll forward", default=1)
    parser.add_argument("-scales", "--multi_scales", type=float, nargs='+', help="scales", default=[])
    ...
    f_nn   = network + "_levels_" + str(levels) + "_" + act_fn + \
            ...
            "_l2" + str(l2_reg) + "_l_sc" + str(loss_scale) + "_l_de" + str(loss_derivative) + "_deb" + str(debug) 
    ...
    for an in ["train", "cv"]:
        ...
        sims_vec[an], times_vec[an] = init_dataset(data_dir, an, is_init=False, debug=debug, roll_forward=roll_forward)

        if debug:
            sims_vec_init[an], times_vec_init[an] = None, None
        else:
            sims_vec_init[an], times_vec_init[an] = init_dataset(data_dir, an, is_init=True, debug=debug, roll_forward=roll_forward)
