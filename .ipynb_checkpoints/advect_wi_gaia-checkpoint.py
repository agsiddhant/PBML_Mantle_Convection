...
if run_cell:
    ...
    parser.add_argument("-r", "--repeats", type=int, help="repeats in hidden layers", default=4)
    parser.add_argument("-k", "--kernel", type=int, help="kernel size", default=5)
    parser.add_argument("-w", "--warm_up_steps", type=int, help="warm_up_steps", default=0)
    parser.add_argument("-i", "--intervene_TS", type=int, help="intervene_TS", default=1)
    parser.add_argument("-t", "--t_end", type=float, help="t_end", default=10)
    parser.add_argument("-m", "--mode", type=str, help="mode", default="GAIA")
    parser.add_argument("-save", "--save_steps",   type=int, help="save_steps", default=200)
    parser.add_argument("-write", "--write_steps", type=int, help="write steps", default=200)
    parser.add_argument("-ad", "--advection_scheme", type=int, help="advection_scheme", default=2)
    ...
    parser.add_argument("-l_de", "--loss_derivative", type=int, help="loss derivative", default=1)
    parser.add_argument("-l2", "--l2_reg", type=float, help="l2_reg", default=0.0)
    parser.add_argument("-net", "--network", type=str, help="network name", default="none")
    ...
    parser.add_argument("-cool",  "--core_cool",         type=int, help="core cooling", default=0)
    parser.add_argument("-decay", "--radioactive_decay", type=int, help="radioactive decay", default=0)
    parser.add_argument("-init", "--initialization", type=str, help="gaia initialization", default="hot")
    parser.add_argument("-sol",  "--solver", type=str, help="momentum solver", default="mumps")
    parser.add_argument("-u",  "--urf", type=float, help="urf if iterative momentum solver", default=1)
...
if args.mode=="GAIA":
    simulation = "raq_" + str(raq) + "_fkt_" + str(fkt) + "_fkv_" + str(fkp) + "_mmskip" + str(intervene_TS
                  ) + "_sol" + solver + "_urf" + str(urf) + "_Di" + str(Di) + "_start" + initialization
else:
    simulation = network + "_raq_" + str(raq) + "_fkt_" + str(fkt) + "_fkv_" + str(fkp) + "_f" + str(
                 c_h) + "_symm" + str(use_symm) + "_r" + str(repeats) + "_k" + str(
                 kernel) + "_p" + str(p_pred) + "_l" + loss_type + "_b" + str(batch_size
                  ) + "_ad" + str(advect) + "_lev" + str(levels) + "_l_sc" + str(loss_scale
                  ) + "_l_de" + str(loss_derivative) + "_l2_" + str(l2_reg
                  ) + "_r_p" + r_p + "_Di" + str(Di) + "_start" + initialization + "_sol" + solver 
...
f_nn   =    network + "_levels_" + str(levels) + "_" + act_fn + \
            ...
            "_l2" + str(l2_reg) + "_l_sc" + str(loss_scale) + \
...
if "ML" in modes or "ML_STOKES" in modes or "ML_PRE" in modes:
    ...
    model_uvp = get_model(c_h, repeats, kernel, use_symm, p_pred, network).double().to(device)
...
for mode in modes:
    ...
    if mode=="ML":
        ...
        ts_net = TS(model_uvp, model_AD, device=device, ts=1, 
                    advection_scheme=advection_scheme, scale=scale, p_pred=p_pred, net=network).double().to(device)
        print(ts_net)
    ...
    create_ini_file(f_gaia_ini, mode, raq.item(), fkt.item(), fkp.item(), 
                    advection_scheme, intervene_TS, warm_up_steps, 
                    solver=solver, initialization=initialization, urf=urf, Di=Di, 
                    core_cool=core_cool, radioactive_decay=radioactive_decay, CaseID=CaseID)
    ...
    def attempt(t, n_step):
        ...
        while t<t_end:
            ...
            if mode != "GAIA" and mode != "GAIA_PRECONDITIONER": 
                ...
                t0_nn  = time.time()
                T_new, dts, u,v,p,V = ts_net(Tp, sdf, sdf2, ycc, raq_nd, fkt_nd, fkp_nd, raq, fkt, fkp, xcc, ycc)
                ...
                state["v"][:,:] = np.copy(np.concatenate((u.reshape(-1,1),
                                                  v.reshape(-1,1),
    ...
    def attempt_unet(t, n_step):
        ...
        state  = sim.getState()
        Tp     = torch.tensor(state["T"], dtype=torch.float64).view(1,1,128,506)
        up     = torch.tensor(state["v"][:,0], dtype=torch.float64).view(1,1,128,506)/scaler
        vp     = torch.tensor(state["v"][:,1], dtype=torch.float64).view(1,1,128,506)/scaler
        ...
        while t<t_end:
            ...
            state = sim.getState()
            dx_min = 0.5/126.
            uv_mag = torch.max(torch.amax(torch.abs(up*scaler)), torch.amax(torch.abs(vp*scaler)))
            dt_advect = 0.5* 100. * dx_min/uv_mag    # 0.5 * CN_max
            dt_diffuse = 0.5*((dx_min*dx_min)**2)/(dx_min**2 + dx_min**2)  
            dt = torch.tensor(np.min([dt_advect.item(),dt_diffuse]), dtype=torch.float64).expand(1,1,vp.shape[-2],vp.shape[-1])
        
            t0_nn  = time.time()
            T_new, dts, u,v,p,V = ts_net(Tp, sdf, sdf2, ycc, raq_nd, fkt_nd, fkp_nd, raq, fkt, fkp, xcc, ycc, up, vp, dt)
