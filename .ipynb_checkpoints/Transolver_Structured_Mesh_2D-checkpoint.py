...
class Model(nn.Module):
    def __init__(self,
        ...
        if self.unified_pos:
            ...
            self.preprocess = MLP(fun_dim + self.ref_x * self.ref_y, 
    ...
    def get_grid(self, batchsize=1):
        ...
        gridx = torch.tensor(np.linspace(0, 1, self.ref_x), dtype=torch.float)
        gridx = gridx.reshape(1, self.ref_x, 1, 1).repeat([batchsize, 1, self.ref_y, 1])
        gridy = torch.tensor(np.linspace(0, 1, self.ref_y), dtype=torch.float)
        gridy = gridy.reshape(1, 1, self.ref_y, 1).repeat([batchsize, self.ref_x, 1, 1])
        grid_ref = torch.cat((gridx, gridy), dim=-1).cuda()  # B H W 8 8 2

        pos = torch.sqrt(torch.sum((grid[:, :, :, None, None, :] - grid_ref[:, None, None, :, :, :]) ** 2, dim=-1)). \
            reshape(batchsize, size_x, size_y, self.ref_x*self.ref_y).contiguous()
        return pos

    def forward(self, data, T=None): #x, fx, T=None):
        x  = data[:,:,:2]
        fx = data[:,:,2:]
        
        if self.unified_pos:
            x = self.pos.repeat(x.shape[0], 1, 1, 1).reshape(x.shape[0], self.H * self.W, self.ref_x*self.ref_y)
        if fx is not None:
            fx = torch.cat((x, fx), -1)
        ...
        fx = fx.view(-1,128,506,self.out_dim)
        a  = fx[...,0:1]*self.a_bound #nn.Tanh()(y[:,0:1,...])
