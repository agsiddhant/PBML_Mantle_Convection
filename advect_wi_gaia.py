...
gaia_dir = gaia_dir_pre + "new_" + simulation + "/"
...
for f in fs:
    ...
    if f == "ini":
        os.symlink(gaia_dir_pre + f, gaia_dir + f_o)
    else:
        shutil.copyfile(gaia_dir_pre + f, gaia_dir + f_o)
...
if "ML" in modes or "ML_STOKES" in modes or "ML_PRE" in modes:
    ...
    model_uvp.load_state_dict(
        torch.load(_nn_dir + str(epoch) + "_fluidnet_uvp.pt", map_location=device)
    )
    model_uvp.eval()
...
# In[ ]:
H_U_238  = torch.tensor(raq, dtype=torch.float64)
...
for mode in modes:
    ...
    if mode == "ML":
        model_AD = ADNet(device=device, CN_max=0.49).double().to(device)
        model_AD.eval()
        ts_net = (
            TS(
                model_uvp,
    ...
    def attempt(t, n_step):
        ...
        while t < t_end+2*dt:
            ...
            if radioactive_decay:
                ...
                lambda_U_238 = 38.4
                C_U_0 = 1.0
                raq = torch.tensor(C_U_0 * H_U_238  * np.exp(-t * lambda_U_238) , dtype=torch.float64)
                
            else:
                raq = torch.tensor(H_U_238 , dtype=torch.float64)
            ...
            if mode != "GAIA" and mode != "GAIA_PRECONDITIONER":
                ...
                if mode == "ML" and n_step % intervene_TS != 0:
                    Tp = T_new[1].clone()
                    Tmean = torch.mean(Tp).detach().cpu().numpy()
                    
                    dt = dts[1].clone().detach().cpu().numpy()
                    #state["T"][:] = np.copy(Tnew)
                    print(len(T_vec[mode]), t, dt, Tmean)
            ...
            if t > save_t:
                ...
                if mode=="ML":
                    ...
                    uv = np.copy(
                        np.concatenate(
                            (
                                u.detach().cpu().numpy().reshape(-1, 1),
                                v.detach().cpu().numpy().reshape(-1, 1),
                                np.zeros_like(u.detach().cpu().numpy().reshape(-1, 1)),
                            ),
                            axis=1,
                        )
                    )
                    snapshots[mode]["v"].append(uv)
                    snapshots[mode]["T"].append(np.copy(Tnew))
                else:
                    for var in ["v", "P", "T"]:
                        snapshots[mode][var].append(np.copy(state[var]))
