...
def get_sdf(x, y):
    """
    Computes the signed distance function (SDF) for the given coordinates.
    ...
    Returns:
        torch.Tensor: The signed distance function values for the given coordinates.
    """
    x_min = x.min()
    x_max = x.max()
    y_min = y.min()
...
def eta(gamma, beta, z, T, Tref=0, zref=0):
    """
    Computes viscosity using frank-kamenetskii.

    Args:
        gamma (float): viscosity constrast due to temperature.
        beta (float): viscosity constrast due to depth.
        z (float):  depth (1-y coordinate).
        T (float):  temperature.
        Tref (float, optional): Reference temperature. Defaults to 0.
        zref (float, optional): Reference depth. Defaults to 0.
...
def get_indices_time(data_dir, an, is_init=False, debug=True, roll_forward=1):
    """
    Retrieves simulation and time indices for training or validation of time-learning Unet.

    Args:
        data_dir (str): Directory containing simulation data.
        an (str): Dataset type ('train' or 'cv').
        is_init (bool, optional): Whether to retrieve initial indices. Defaults to False.
        debug (bool, optional): Whether to enable debug mode. Defaults to True.
        roll_forward (int, optional): Number of steps to roll forward. Defaults to 1.

    Returns:
        tuple: A tuple containing:
            - sims_vec (list): List of simulation indices.
            - times_vec (list): List of corresponding time indices.
    """
...
class ADTimeDataset(Dataset):
    ...
    """
    A PyTorch Dataset for time-dependent simulation data.

    This dataset handles loading and preprocessing of simulation data for training
    and validation. It supports scaling, noise addition, and rolling forward through time.

    Args:
        data_dir (str): Directory containing simulation data.
        an (str): Dataset type ('train' or 'cv').
        scale (bool, optional): Whether to scale the data. Defaults to True.
        load (bool, optional): Whether to load preprocessed data. Defaults to False.
        is_init (bool, optional): Whether to retrieve initial indices. Defaults to False.
        p_pred (bool, optional): Whether to include pressure predictions. Defaults to True.
        noise (float, optional): Amount of noise to add to the data. Defaults to 0.0.
        debug (bool, optional): Whether to enable debug mode. Defaults to True.
        sims_vec (list, optional): List of simulation indices to include. Defaults to [].
        times_vec (list, optional): List of time indices to include. Defaults to [].
        roll_forward (int, optional): Number of steps to roll forward. Defaults to 1.

    Methods:
        - __len__: Returns the number of examples in the dataset.
        - __getitem__: Retrieves a single example from the dataset.
    """
...
def get_indices(data_dir, an, is_init=False, debug=True, roll_forward=1):
    """
    Retrieves simulation and time indices for Stokes surrogate.
...
class NewADDataset(Dataset):
    """
    A PyTorch Dataset for Stokes surrogate.

    This dataset handles loading and preprocessing of simulation data for training
    and validation. It supports scaling, noise addition, and limiting the number of
    examples per epoch.
    ...
    Args:
        ...
        scale (bool, optional): Whether to scale the data. Defaults to True.
        load (bool, optional): Whether to load preprocessed data. Defaults to False.
        is_init (bool, optional): Whether of type initial time-step indices. Defaults to False.
        p_pred (bool, optional): Whether to include pressure predictions. Defaults to True.
        noise (float, optional): Amount of noise to add to the data. Defaults to 0.0.
        debug (bool, optional): Whether to enable debug mode. Defaults to True.
        sims_vec (list, optional): List of simulation indices to include. Defaults to [].
        times_vec (list, optional): List of time indices to include. Defaults to [].
        max_examples_percent_per_epoch (int, optional): Maximum percentage of examples
            to use per epoch. Defaults to 100.
