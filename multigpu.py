...
def ddp_setup(rank, world_size, master_port):
    """
    Sets up the Distributed Data Parallel (DDP) environment for multi-GPU training.

    Args:
        rank (int): Unique identifier of each process (GPU).
        world_size (int): Total number of processes (GPUs) participating in training.
        master_port (int): Port number for communication between processes.

    Environment Variables:
        MASTER_ADDR: Address of the master node (set to "localhost").
        MASTER_PORT: Port for communication between processes.
    """
    #print(rank)
...
class Trainer:
    def __init__(
        ...
        Args:
            model_uvp (torch.nn.Module): The primary model for training.
            model_AD (torch.nn.Module): The advection-diffusion model if enabled.
            train_data (DataLoader): DataLoader for the training dataset.
            cv_data (DataLoader): DataLoader for the cross-validation dataset.
            train_data_init (DataLoader): DataLoader for the initial training dataset.
            cv_data_init (DataLoader): DataLoader for the initial cross-validation dataset.
            optimizer (torch.optim.Optimizer): Optimizer for training the models.
            scheduler (torch.optim.lr_scheduler): Learning rate scheduler.
            gpu_id (int): ID of the GPU to use for training.
            save_every (int): Frequency (in epochs) to save model checkpoints.
            nn_dir (str): Directory to save model checkpoints and logs.
            p_pred (bool, optional): Flag to enable prediction mode. Defaults to False.
            debug (bool, optional): Flag to enable debug mode. Defaults to False.
            network (str, optional): Name of the network architecture. Defaults to "fluidnet".
            loss_scale (bool, optional): Flag to enable loss scaling. Defaults to False.
            loss_derivative (bool, optional): Flag to enable derivative loss computation. Defaults to False.
            roll_forward (int, optional): Number of steps to roll forward during training. Defaults to 1.
            epoch (int, optional): Starting epoch number. Defaults to 0.
            loss_type (str, optional): Type of loss function to use. Defaults to "curl".
        """
        self.gpu_id = gpu_id
    ...
    def loss_fn(self, x_true, x_pred):
        ...
        if self.loss_scale:
            ...
            l_scaler = torch.clip(1.0 / (maxs - mins), 1.0, 10.0)
            bc_scaler = torch.ones_like(x_true) + 10.0
            bc_scaler[:, 2:-2, 2:-2] = 1.0
            loss = torch.mean(torch.abs(((x_true - x_pred)) * l_scaler * bc_scaler))
    ...
    def get_loss(self, gVTp, uvp, scaler, paras=None, yc=None):
        ...
        if "fluidnet" in self.net:
            ...
            du_dx = F.conv2d(u, self.dx_center_kernel)[..., 1:-1, :] * scaler.to(self.gpu_id) * 126
            dv_dy = F.conv2d(v, self.dy_center_kernel)[..., :, 1:-1] * scaler.to(self.gpu_id) * 126
...
def load_train_objs(
    ...
    """
    Loads the necessary objects for training.

    This function is responsible for loading models, optimizers, datasets,
    and any other components required to resume or initialize training.
...
def prepare_dataloader(dataset: Dataset, batch_size: int, world_size, rank):
    """
    Prepares and returns a PyTorch DataLoader object for the given dataset.

    This function sets up the DataLoader instance for the training or validation
    dataset, including configurations such as batch size, pin memory, and shuffling.

    Args:
        dataset (Dataset): The dataset to load data from.
        batch_size (int): The number of samples per batch.
        world_size (int): The total number of processes (GPUs) for distributed training.
        rank (int): The rank of the current process (GPU).

    Returns:
        DataLoader: A PyTorch DataLoader instance for the given dataset.
    """
    return DataLoader(
        dataset,
...
def main(
    ...
    """
    A generic function for configuring and executing a training or simulation process.
    ...
    Args:
        ...
        levels (int): Number of levels in the model or simulation.
        ...
        r_p (float): padding type. "learned" or "zeros"
        loss_type (str): Type of loss function to use. "curl" for hard mass conservation or "mass" for soft.
        use_symm (bool): Whether to use symmetry in the model or simulation.
        repeats (int): Number of repetitions for convolutionss.
        kernel (int): Kernel size for convolutional layers.
        milestones (list): Epoch milestones for learning rate adjustments.
        sims_vec (list): Vector of simulation parameters.
        times_vec (list): Vector of time parameters for simulations.
        sims_vec_init (list): Initial time-steps vector of simulation parameters.
        times_vec_init (list): Initial time-steps vector of time parameters for simulations.
        use_skip (bool, optional): Whether to use skip connections. Defaults to False.
        p_pred (bool, optional): Whether to enable prediction mode. Defaults to False.
        spectral_conv (bool, optional): Whether to use spectral convolution. Defaults to False.
        dilation (int, optional): Dilation rate for convolutional layers. Defaults to 1.
        a_bound (int, optional): Boundary value for certain operations. Defaults to 10.
        restart (bool, optional): Whether to restart training from a checkpoint. Defaults to False.
        advect (bool, optional): Whether to enable advection during training. Defaults to False.
        network (str, optional): Name of the network architecture. Defaults to "fluidnet".
        debug (bool, optional): Whether to enable debug mode. Defaults to False.
        scale (bool, optional): Whether to scale input data. Defaults to True.
        blurr (bool, optional): Whether to apply blurring to input data. Defaults to False.
        master_port (int, optional): Port number for distributed training communication. Defaults to 366.
        l2_reg (float, optional): L2 regularization parameter. Defaults to 0.0.
        dropout (float, optional): Dropout rate for the model. Defaults to 0.0.
        loss_scale (bool, optional): Whether to enable loss scaling. Defaults to False.
        loss_derivative (bool, optional): Whether to compute derivative loss. Defaults to False.
        roll_forward (int, optional): Number of steps to roll forward during training. Defaults to 1.
        factor (int, optional): Scaling factor for certain operations. Defaults to 2.
        multi_scales (list, optional): List of scales for multi-scale processing. Defaults to [].

    Returns:
        Any: The return value depends on the function's implementation.
    """
